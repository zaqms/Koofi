...
| `meet_halfway_results` | ثلاث قهاوي بينكم (or leftover 1–2) shown | **Keep** `locale`, `count`, `source` (`local` / `invite`). **Additive:** `cafes[]` (`name_ar`, `name_en`, `district`, `maps_url`, optional `distance_km`), and `pack_id` when an `/h/{id}` session exists. Do not send lat/lng. Email is **not** a GTM tag — see server webhook below. |
...
### بيننا results email (server webhook — not GTM)
...
`distance_km` is omitted when pin or café coords are missing. `session_id` is omitted on a local two-pin run with no `/h/{id}`. `notify_email` is locked to **aj@cali.sa** (not amjad@cali.sa).
