...
`robots.txt` allows `/api/shops`, `/api/mcp`, `/mcp`, and `/llms.txt`; other `/api/` routes stay disallowed. The sitemap is the committed static `public/sitemap.xml` (Replit urlset for GSC). `npm run generate-sitemap` / `prebuild` verify that file; they do not regenerate it from the catalog.
