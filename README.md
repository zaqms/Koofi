...
- **بيننا / Halfway.** A chip on the vibe row. Three first-class screens: **Invite** (one Ready location — `موقعي` / already-granted geo auto-Ready, Maps-share paste if denied — then **اعزم خويك** / Copy link), **Waiting** (`ننتظر دبوس صاحبك. الثلاث تظهر هنا لحالها.` / `Waiting for your friend’s pin — three cafes will appear here.`), **Results** (`ثلاث قهاوي بينكم`). Hero is pin–cup–pin (one cup). Lat/lng stay off the UI. Guest `/h/{token}?from=wa` reuses the Invite pattern (their pin only) and auto-Ready when the browser already granted location — never copies the friend’s pin; Maps paste stays if permission is prompt/denied/off. A's host page polls the invite overlay and shows **صاحبك دبّس.** then the three around the pin centroid, ranked with the same Most Popular index. Core API is `locations: Location[]` (N≥2); v1 UI is two people. Composer + أضف قهوة stay hidden while the flow is open. Soft Places stays parked.
