...
- Two landings, same chat: **`/`** is Arabic (`وين ودّك تروح اليوم؟`, Arabic chips, RTL). **`/en`** is English (`Where do you want to go today?`, English chips, LTR). Header switches with `EN` / `عربي`. Not a marketing site. Not both languages stacked in one bubble.
- **بيننا** is a utility card above an 8-chip 4×2 home grid (`popular` selected by default). Off-home chip URLs (`roaster`, `specialty`, `study`, `late`) stay live. Tapping a vibe chip still returns **three** pick cards. Soft Places stays parked. Typing the other language still flips the reply. WhatsApp has no chip UI; typing the same Arabic or English phrase maps the same way.
...
- **بيننا / Halfway.** A utility card above the home chips (not a grid tile). One share URL `wain.lol/h/{id}` is the whole journey (`/en/h/{id}` is the English twin — same session, same pin state machine): **Invite** (one Ready location — `موقعي` / already-granted geo auto-Ready, Maps-share paste if denied — then **اعزم خويك** / Copy link), **Waiting** (`ننتظر دبوس صاحبك. الثلاث تظهر هنا لحالها.` / `Waiting for your friend’s pin — three cafes will appear here.`), **Results** (`ثلاث قهاوي بينكم`). After اعزم خويك the host `replace`s onto `/h/{id}` (EN host: `/en/h/{id}`) so host and guest share the session. Overlay GET: 1 pin → Waiting (host) or Invite (guest); 2+ pins or frozen `shop_ids` → the same three cards (refresh does not reshuffle). Waiting TTL is 45 minutes; completed sessions live 48 hours after results freeze. Expired shows clear copy plus **Start a new Halfway** — not a silent restart. Share results is the same `/h/{id}` (not `/p/`). No Save for later. Hero is pin–cup–pin (one cup). Lat/lng stay off the UI. Guest never copies the friend’s pin. Core API is `locations: Location[]` (N≥2); v1 UI is two people. Composer + أضف قهوة stay hidden while the flow is open. Soft Places stays parked.
...
- Riyadh only. Neighborhoods: Hittin (حطين), Al Malqa (الملقا), An Nakheel (النخيل), Al Yasmin (الياسمين), Al Olaya (العليا), As Sulimaniyah (السليمانية), Al Wurud (الورود), Ar Rabwah (الربوة), Ar Rabi (الربيع), Al Masif (المصيف), Ar Rahmaniyyah (الرحمانية). Empty dictionary-only districts stay out of live discovery.
...
| `invite_open` | وين؟ / wain? sheet opened (parked — unused while `SHOW_INVITE_CTA` is false) | `shop_id`, `locale` |
| `invite_share` | وين؟ / wain? shared (parked) | `shop_id`, `locale`, `channel` |
...
4. Preview on `/c/woods-olaya` (Passport) and `/c/cafu-olaya` (thin). `invite_*` and `tonight_*` can stay tagged but will not fire while parked. Publish the container.
...
Email `maps_url` hrefs prefer an existing `maps.app.goo.gl` (or `goo.gl`) shortlink already on the café. Six catalog rows store an Amjad/Ajz share link (Bacha Solitaire, Mood Masters, LAS Malqa, LAS Olaya, Little Henri, Beitkull); those email hrefs stay the shortlink. Every other `mapsShareUrl` is `https://www.google.com/maps/…`, so the mailer gets a first-party `https://wain.lol/go/maps?shop={id}` hop that 302s onto that place URL. Pins have no stored shortlink; they use `https://wain.lol/go/maps?lat={lat}&lng={lng}`. That keeps Gmail from wrapping a raw `google.com/maps` href as `google.com/url?q=https://www.google.com/maps/…` (the double-google confirm). Product like / share / Maps buttons and the GTM `cafes[].maps_url` stay on the catalog href.
...
lib/tonight.ts                  parked وين؟ / wain? invite + Tonight mint, session rate limit
components/viral-share.tsx      parked وين؟ / wain? share sheet (Passport + thin)
