...
- **بيننا / Halfway.** A chip on the vibe row. Pick two live catalog districts (`أنا في` / `الثاني`). Returns **exactly three** live-catalog cafes in a band around the centroid of those district centroids, ranked with the same Most Popular index. Core API is `locations: Location[]` (N≥2); v1 UI is two district pickers. No pins, no radar map. Soft Places stays parked.
