...
- **بيننا / Halfway.** A chip on the vibe row. Paste two shared pins (Google Maps link, lat,lng, or “موقعي”). Returns **exactly three** live-catalog cafes in a band around the centroid of those pins, ranked with the same Most Popular index. Core API is `locations: Location[]` (N≥2); v1 UI is two pins. No radar map. Soft Places stays parked.
