...
- **بيننا / Halfway.** A chip on the vibe row. Two shared location pins (Google Maps share/place URL, lat-lng, and/or browser geolocation). Returns **exactly three** live-catalog cafes in a band around the centroid of those pins, ranked with the same Most Popular index. Core API is `locations: Location[]` (N≥2); v1 UI is two pins. Soft Places stays parked.
