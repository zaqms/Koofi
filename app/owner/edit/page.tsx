...
export async function generateMetadata() {
  return {
    title: `${copy.ownerEditTitle.ar} · ${PRODUCT_NAME}`,
    robots: { index: false, follow: false },
  };
}

export default async function OwnerEditPage({ searchParams }: OwnerEditPageProps) {
  ...
  if (!session.ok || !token) {
    return <OwnerEditDenied language="ar" error={session.ok ? "missing" : session.error} />;
  }
