...
assert(
  Object.keys(popularityIndex).length === 375,
  `popularity map should have 375 ids, got ${Object.keys(popularityIndex).length}`,
);
assert(
...
assert(
  ...
  "For two is not a live with-friends alias",
);
assert(
  !parseIntent("لاثنين").moments.includes("with-friends"),
  "لاثنين is not a live with-friends alias",
);
assert(
  !parseIntent("Good for a date").moments.includes("with-friends"),
  "old EN dating label is not a live alias",
);
assert(
  !parseIntent("لموعد").moments.includes("with-friends"),
  "old AR dating label is not a live alias",
);
assert(
